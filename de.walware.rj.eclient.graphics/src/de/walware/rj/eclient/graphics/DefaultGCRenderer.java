...
//					continue;
			...
			case RGraphicInstruction.DRAW_TEXT: {
				...
				if (element.rotateDegree != 0.0) {
					...
					transform.translate((float) (element.x * scale), (float) (element.y * scale));
					transform.rotate((float) -element.rotateDegree);
					if (hShift != 0.0) {
						transform.translate((float) - hShift, - gc.getFontMetrics().getAscent());
					}
